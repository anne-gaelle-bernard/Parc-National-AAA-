...
const init = () => {
  ...
  // --- Données (simulées) ---
  // Removed hardcoded campings array
      ...
      fetch(`/Parc-National-AAA-/Backend/api/get-camping-details.php?id=${campingId}`)
          ...
          .then(data => {
              if (data.status === 'success' && data.camping) {
                  ...
                  mainContent.innerHTML = "<p>Camping non trouvé.</p>";
              }
          })
          .catch(error => {
              console.error("Error fetching camping details:", error);
              mainContent.innerHTML = "<p>Une erreur est survenue lors de la récupération des détails du camping.</p>";
          });
  };
